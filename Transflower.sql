...
create table if not exists emp_roles (
 ...
 constraint fk_emp foreign key (empid) references employees(id) on update CASCADE on delete CASCADE,
...
/*  Get all employees based on role name set */
select * from employees
where id IN(select  empid from emp_roles
...
select rolename from roles where roleid IN(select roleid from emp_roles where empid=1);